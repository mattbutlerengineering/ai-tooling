...
**Tier 1 — measured (20): install with confidence.** Run hands-on, with metrics (`MEASURED`) or at least a smoke test (`RUN`).

context7 (RUN), GSD (MEASURED), github-mcp-server (MEASURED), codegraph (MEASURED), markitdown (MEASURED), caveman (MEASURED), headroom (MEASURED), claude-squad (RUN), beads (MEASURED), playwright (RUN), stryker-js (RUN), security-guidance (MEASURED), claude-code-action (RUN), resolving-merge-conflicts (MEASURED), documentation-and-adrs (MEASURED), last30days (MEASURED), serena (MEASURED), web-quality-skills (MEASURED), fastmcp (RUN), skill-creator (MEASURED)

**Tier 2 — review-based (10): try at your own risk.** Evaluated by reading docs/source (`REVIEW`) or inferred from catalog metadata (`SOURCE-ONLY`), not run hands-on — graduating an eval ([#68](https://github.com/mattbutlerengineering/ai-tooling/issues/68)) promotes a tool to Tier 1.

feature-dev (REVIEW), agent-skills (REVIEW), mattpocock/skills (SOURCE-ONLY), code-review (REVIEW), pr-review-toolkit (REVIEW), trailofbits/skills (REVIEW), claude-reflect (SOURCE-ONLY), claude-mem (SOURCE-ONLY), abtop (SOURCE-ONLY), SkillSpector (REVIEW)
<!-- TIERS:END -->
