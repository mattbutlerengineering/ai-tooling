...
The 607 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 109 distinct values across these 607 leads (262 have zero overlap pressure; largest tie: 46) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P2 challenger** | overlaps a tool already in STACK | 180 | SKIP "redundant with `<incumbent>`", or leave at discovery-log |
| **P3 backlog** | everything else | 397 | leave; stamp `**Last triaged:**` only |
...
|------|-------|-------|-----|---------|
| cognee | Memory & Context | 50.7 | pressure 21, gap 6.7 | `/evaluate-tool cognee` |
...
| OpenHands | Implement | 37.7 | pressure 15, gap 5.7 | `/evaluate-tool OpenHands` |
| goose | Implement | 37.7 | pressure 15, gap 5.7 | `/evaluate-tool goose` |
| supermemory | Memory & Context | 34.7 | pressure 13, gap 6.7 | `/evaluate-tool supermemory` |
| sandcastle | Implement | 33.7 | pressure 13, gap 5.7 | `/evaluate-tool sandcastle` |
...
| vet | Review | 53.1 | pressure 22, gap 7.1 | `/evaluate-tool vet` |
| orca | Implement | 41.7 | pressure 17, gap 5.7 | `/evaluate-tool orca` |
| aider | Implement | 39.7 | pressure 17, gap 5.7 | `/evaluate-tool aider` |
| ghostsecurity/skills | Review | 37.1 | pressure 14, gap 7.1 | `/evaluate-tool ghostsecurity/skills` |
| gastown | Implement | 35.7 | pressure 14, gap 5.7 | `/evaluate-tool gastown` |
...
| browser-use | Verify | 32.3 | pressure 12, gap 6.3 | `/evaluate-tool browser-use` |
| ralph-claude-code | Implement | 31.7 | pressure 12, gap 5.7 | `/evaluate-tool ralph-claude-code` |
| worktrunk | Ship | 31.5 | pressure 11, gap 7.5 | `/evaluate-tool worktrunk` |
| claude-octopus | Review | 29.1 | pressure 10, gap 7.1 | `/evaluate-tool claude-octopus` |
...
## P2 challenger — 180 leads

_SKIP "redundant with `<incumbent>`", or leave at discovery-log._

_Listing 12 of 180 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._

| Tool | Stage | Score | Why | Command |
|------|-------|-------|-----|---------|
| engram | Memory & Context | 28.7 | challenges claude-mem · pressure 10, gap 6.7 | `/triage-lead engram` |
| ruflo | Implement | 27.7 | challenges GSD · pressure 10, gap 5.7 | `/triage-lead ruflo` |
...
| gstack | Implement | 25.7 | challenges GSD · pressure 9, gap 5.7 | `/triage-lead gstack` |
| memU | Memory & Context | 24.7 | challenges claude-mem · pressure 9, gap 6.7 | `/triage-lead memU` |
| compound-engineering | Implement | 23.7 | challenges GSD · pressure 8, gap 5.7 | `/triage-lead compound-engineering` |
...
| agnix | Review | 21.1 | challenges SkillSpector · pressure 6, gap 7.1 | `/triage-lead agnix` |
...
## P3 backlog — 397 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 397 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._

| Tool | Stage | Score | Why | Command |
|------|-------|-------|-----|---------|
| claude-code-router | Implement | 27.7 | pressure 10, gap 5.7 | `/triage-lead claude-code-router` |
| gptme | Implement | 25.7 | pressure 9, gap 5.7 | `/triage-lead gptme` |
| qwen-code | Implement | 25.7 | pressure 9, gap 5.7 | `/triage-lead qwen-code` |
...
| CLIProxyAPI | Implement | 23.7 | pressure 9, gap 5.7 | `/triage-lead CLIProxyAPI` |
| gemini-cli | Implement | 23.7 | pressure 8, gap 5.7 | `/triage-lead gemini-cli` |
