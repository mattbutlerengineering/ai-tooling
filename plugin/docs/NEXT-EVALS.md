...
The 481 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 102 distinct values across these 481 leads (209 have zero overlap pressure; largest tie: 33) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P2 challenger** | overlaps a tool already in STACK | 141 | SKIP "redundant with `<incumbent>`", or leave at discovery-log |
| **P3 backlog** | everything else | 310 | leave; stamp `**Last triaged:**` only |
...
| agentmemory | Memory & Context | 32.5 | pressure 12, gap 6.5 | `/evaluate-tool agentmemory` |
...
| ACE (agentic-context-engine) | Memory & Context | 26.5 | pressure 9, gap 6.5 | `/evaluate-tool ACE (agentic-context-engine)` |
| memU | Memory & Context | 26.5 | pressure 9, gap 6.5 | `/evaluate-tool memU` |
...
## P2 challenger — 141 leads

_SKIP "redundant with `<incumbent>`", or leave at discovery-log._

_Listing 12 of 141 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
...
## P3 backlog — 310 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 310 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
