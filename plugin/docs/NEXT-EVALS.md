...
The 599 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 108 distinct values across these 599 leads (259 have zero overlap pressure; largest tie: 45) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P2 challenger** | overlaps a tool already in STACK | 178 | SKIP "redundant with `<incumbent>`", or leave at discovery-log |
| **P3 backlog** | everything else | 391 | leave; stamp `**Last triaged:**` only |
...
|------|-------|-------|-----|---------|
| cognee | Memory & Context | 48.9 | pressure 20, gap 6.9 | `/evaluate-tool cognee` |
| langfuse | Outer Loop | 43.4 | pressure 18, gap 7.4 | `/evaluate-tool langfuse` |
| mem0 | Memory & Context | 40.9 | pressure 16, gap 6.9 | `/evaluate-tool mem0` |
...
| ghostsecurity/skills | Review | 35.1 | pressure 13, gap 7.1 | `/evaluate-tool ghostsecurity/skills` |
| agentmemory | Memory & Context | 34.9 | pressure 13, gap 6.9 | `/evaluate-tool agentmemory` |
...
| ACE (agentic-context-engine) | Memory & Context | 28.9 | pressure 10, gap 6.9 | `/evaluate-tool ACE (agentic-context-engine)` |
...
## P2 challenger — 178 leads

_SKIP "redundant with `<incumbent>`", or leave at discovery-log._

_Listing 12 of 178 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
...
## P3 backlog — 391 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 391 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._

| Tool | Stage | Score | Why | Command |
|------|-------|-------|-----|---------|
| claude-code-router | Implement | 25.6 | pressure 9, gap 5.6 | `/triage-lead claude-code-router` |
