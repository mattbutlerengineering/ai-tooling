...
The 629 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 95 distinct values across these 629 leads (267 have zero overlap pressure; largest tie: 55) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P2 challenger** | overlaps a tool already in STACK | 189 | SKIP "redundant with `<incumbent>`", or leave at discovery-log |
| **P3 backlog** | everything else | 410 | leave; stamp `**Last triaged:**` only |
...
| vet | Review | 57.3 | pressure 24, gap 7.3 | `/evaluate-tool vet` |
| orca | Implement | 41.8 | pressure 17, gap 5.8 | `/evaluate-tool orca` |
| aider | Implement | 39.8 | pressure 17, gap 5.8 | `/evaluate-tool aider` |
| ghostsecurity/skills | Review | 37.3 | pressure 14, gap 7.3 | `/evaluate-tool ghostsecurity/skills` |
...
| claude-octopus | Review | 31.3 | pressure 11, gap 7.3 | `/evaluate-tool claude-octopus` |
...
## P2 challenger — 189 leads

_SKIP "redundant with `<incumbent>`", or leave at discovery-log._

_Listing 12 of 189 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
...
| agnix | Review | 23.3 | challenges SkillSpector · pressure 7, gap 7.3 | `/triage-lead agnix` |
| garak | Outer Loop | 21.4 | challenges SkillSpector · pressure 6, gap 7.4 | `/triage-lead garak` |
| skill-scanner | Review | 21.3 | challenges SkillSpector · pressure 7, gap 7.3 | `/triage-lead skill-scanner` |
| Skill_Seekers | Skills & Plugins | 20.7 | challenges skill-creator · pressure 6, gap 6.7 | `/triage-lead Skill_Seekers` |

## P3 backlog — 410 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 410 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
