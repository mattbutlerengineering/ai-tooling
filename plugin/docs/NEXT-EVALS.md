...
Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 101 distinct values across these 460 leads (196 have zero overlap pressure; largest tie: 30) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P3 backlog** | everything else | 290 | leave; stamp `**Last triaged:**` only |
...
## P3 backlog — 290 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 290 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
...
| Helicone | Outer Loop | 20.8 | pressure 6, gap 6.8 | `/triage-lead Helicone` |
