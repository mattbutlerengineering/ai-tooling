...
The 378 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 96 distinct values across these 378 leads (146 have zero overlap pressure; largest tie: 24) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P3 backlog** | everything else | 246 | leave; stamp `**Last triaged:**` only |
...
|------|-------|-------|--------------------|---------|
| opencode | Implement | 55.7 | pressure 24, gap 5.7 | `/evaluate-tool opencode` |
...
| ECC | Implement | 35.7 | pressure 14, gap 5.7 | `/evaluate-tool ECC` |
| OpenHands | Implement | 35.7 | pressure 14, gap 5.7 | `/evaluate-tool OpenHands` |
| goose | Implement | 35.7 | pressure 14, gap 5.7 | `/evaluate-tool goose` |
...
| pydantic-ai | Implement | 31.7 | pressure 12, gap 5.7 | `/evaluate-tool pydantic-ai` |
...
| sandcastle | Implement | 27.7 | pressure 10, gap 5.7 | `/evaluate-tool sandcastle` |
...
| agent-kit | Implement | 25.7 | pressure 9, gap 5.7 | `/evaluate-tool agent-kit` |
| aider | Implement | 25.7 | pressure 10, gap 5.7 | `/evaluate-tool aider` |
| gstack | Implement | 25.7 | pressure 9, gap 5.7 | `/evaluate-tool gstack` |
...
| orca | Implement | 25.7 | pressure 9, gap 5.7 | `/triage-lead orca` |
| ruflo | Implement | 25.7 | pressure 9, gap 5.7 | `/triage-lead ruflo` |
...
| gastown | Implement | 23.7 | pressure 8, gap 5.7 | `/triage-lead gastown` |
...
## P3 backlog — 246 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 246 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._

| Tool | Stage | Score | Why (pressure/gap) | Command |
|------|-------|-------|--------------------|---------|
| daytona | Implement | 23.7 | pressure 8, gap 5.7 | `/triage-lead daytona` |
| CLIProxyAPI | Implement | 21.7 | pressure 7, gap 5.7 | `/triage-lead CLIProxyAPI` |
| fast-agent | Implement | 21.7 | pressure 7, gap 5.7 | `/triage-lead fast-agent` |
| bifrost | Implement | 19.7 | pressure 6, gap 5.7 | `/triage-lead bifrost` |
| agent-sandbox | Implement | 17.7 | pressure 5, gap 5.7 | `/triage-lead agent-sandbox` |
| claude-code-router | Implement | 15.7 | pressure 4, gap 5.7 | `/triage-lead claude-code-router` |
| tambo | Implement | 13.7 | pressure 4, gap 5.7 | `/triage-lead tambo` |
| UI-TARS-desktop | Implement | 13.7 | pressure 3, gap 5.7 | `/triage-lead UI-TARS-desktop` |
| capa | Implement | 13.7 | pressure 3, gap 5.7 | `/triage-lead capa` |
| claudian | Implement | 13.7 | pressure 3, gap 5.7 | `/triage-lead claudian` |
| codex-plugin-cc | Implement | 13.7 | pressure 3, gap 5.7 | `/triage-lead codex-plugin-cc` |
| nanobot | Implement | 13.7 | pressure 3, gap 5.7 | `/triage-lead nanobot` |
