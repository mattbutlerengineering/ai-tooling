...
Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 104 distinct values across these 480 leads (209 have zero overlap pressure; largest tie: 33) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| worktrunk | Ship | 25.5 | pressure 8, gap 7.5 | `/evaluate-tool worktrunk` |
...
| Tool | Stage | Score | Why | Command |
|------|-------|-------|-----|---------|
| gastown | Implement | 25.2 | challenges claude-squad · pressure 9, gap 5.2 | `/triage-lead gastown` |
...
| CLIProxyAPI | Implement | 21.2 | pressure 8, gap 5.2 | `/triage-lead CLIProxyAPI` |
