...
The 653 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 105 distinct values across these 653 leads (285 have zero overlap pressure; largest tie: 55) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P2 challenger** | overlaps a tool already in STACK | 199 | SKIP "redundant with `<incumbent>`", or leave at discovery-log |
| **P3 backlog** | everything else | 424 | leave; stamp `**Last triaged:**` only |
...
| vet | Review | 61.5 | pressure 26, gap 7.5 | `/evaluate-tool vet` |
...
| claude-octopus | Review | 33.5 | pressure 12, gap 7.5 | `/evaluate-tool claude-octopus` |
...
| claude-code-router | Implement | 29.8 | pressure 11, gap 5.8 | `/evaluate-tool claude-code-router` |
...
## P2 challenger — 199 leads

_SKIP "redundant with `<incumbent>`", or leave at discovery-log._

_Listing 12 of 199 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._

| Tool | Stage | Score | Why | Command |
|------|-------|-------|-----|---------|
| ACE (agentic-context-engine) | Memory & Context | 28.7 | challenges claude-reflect · pressure 10, gap 6.7 | `/triage-lead ACE (agentic-context-engine)` |
...
| mex | Memory & Context | 22.7 | challenges claude-mem · pressure 8, gap 6.7 | `/triage-lead mex` |

## P3 backlog — 424 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 424 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._

| Tool | Stage | Score | Why | Command |
|------|-------|-------|-----|---------|
| CLIProxyAPI | Implement | 25.8 | pressure 10, gap 5.8 | `/triage-lead CLIProxyAPI` |
...
| bifrost | Implement | 21.8 | pressure 7, gap 5.8 | `/triage-lead bifrost` |
