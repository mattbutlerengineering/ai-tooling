...
**Tier 1 — measured (15): install with confidence.** Run hands-on, with metrics (`MEASURED`) or at least a smoke test (`RUN`).

context7 (RUN), GSD (MEASURED), github-mcp-server (MEASURED), codegraph (MEASURED), markitdown (MEASURED), caveman (MEASURED), headroom (MEASURED), claude-squad (RUN), beads (MEASURED), playwright (RUN), stryker-js (RUN), resolving-merge-conflicts (MEASURED), serena (MEASURED), web-quality-skills (MEASURED), fastmcp (RUN)

**Tier 2 — review-based (15): try at your own risk.** Evaluated by reading docs/source (`REVIEW`) or inferred from catalog metadata (`SOURCE-ONLY`), not run hands-on — graduating an eval ([#68](https://github.com/mattbutlerengineering/ai-tooling/issues/68)) promotes a tool to Tier 1.

feature-dev (REVIEW), agent-skills (REVIEW), mattpocock/skills (SOURCE-ONLY), code-review (REVIEW), pr-review-toolkit (REVIEW), trailofbits/skills (REVIEW), security-guidance (REVIEW), claude-code-action (REVIEW), claude-reflect (SOURCE-ONLY), documentation-and-adrs (REVIEW), claude-mem (SOURCE-ONLY), abtop (SOURCE-ONLY), last30days (REVIEW), skill-creator (REVIEW), SkillSpector (REVIEW)
<!-- TIERS:END -->
