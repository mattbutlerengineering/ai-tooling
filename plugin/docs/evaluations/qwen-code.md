...
**discovery-log — tentative read** — SKIP as a *replacement* for the Claude Code harness, but a legitimate **CONDITIONAL** for two narrow uses: (1) as a Claw/acpx-delegated secondary executor driven *from* Claude Code, and (2) for users whose stack is Qwen-centric or who want an open, multi-provider, self-hostable agent.
