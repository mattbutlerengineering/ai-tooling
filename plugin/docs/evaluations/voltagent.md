...
**SKIP — app-building framework, no dev-loop bridge.** A typed TypeScript framework for building production agents, with workflow orchestration and an ops console.

**Its own evaluation says so.** Its recommendation is "adopt if you're **building production agents in TypeScript**", and it concedes the boundary in the next sentence — "for Python-centric or Claude-Code-harness workflows, pydantic-ai or the existing harness path is a closer fit."
