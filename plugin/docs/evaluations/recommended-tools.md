...
**Why it's in the workflow:** As instruction files grow (ACMM L2 is all about encoding preferences), contradictions creep in. This is the only tool that validates instruction file quality across multiple AI coding tools.
...
**Verdict:** KEEP — the confidence-based filtering is exactly what L3 (measurement) needs. Noisy review tools get ignored; this one stays useful.
...
**Why it's in the workflow:** This is the primary continuous improvement mechanism at L3. Without it, corrections are ephemeral — you fix the same mistake in every new session. Claude-reflect turns corrections into persistent rules.
...
**Verdict:** KEEP — solves the most important problem at L3: turning session-level corrections into persistent improvements. The staleness is a concern but the core mechanism works.
...
**Why it's in the workflow:** Only tool in the stack that provides observability into AI agent behavior at scale. At L3 you need quantitative signals — langfuse provides cost tracking, latency monitoring, eval scoring, and prompt versioning. Nothing else in the stack covers production-grade observability.
...
**Why it's in the workflow:** At L4, agents run longer autonomous sessions with more tool calls. Context window exhaustion becomes the primary bottleneck. Headroom directly addresses this by compressing the inputs that fill context fastest (verbose tool output, log dumps, large file reads).
...
**Verdict:** KEEP — directly solves the L4 constraint (longer autonomous sessions need more context headroom). The 29K stars and active maintenance validate the approach.
...
**Why it's in the workflow:** At L4, you start running concurrent AI sessions via git worktrees (the approach used in the ACMM paper's L4 case study). Raw `git worktree` commands are error-prone and tedious. Worktrunk wraps them into a clean workflow.
...
**Verdict:** CONDITIONAL — keep if you run parallel agent sessions via worktrees. If you only run one agent at a time, skip until L5. The proprietary license is a risk factor.
...
**Why it's in the workflow:** At L4, tasks get complex enough to need structured project management — not just "fix this bug" but "implement this milestone across multiple phases with verification at each step." GSD provides that structure with built-in verification that prevents drift.
...
**Verdict:** KEEP — the structured verification loop is exactly what L4 needs. Use `/gsd:quick` for smaller tasks and the full framework for multi-phase work.
...
**ACMM Level:** L5
**Recommended at:** L5 — Semi-Automated
...
**Why it's in the workflow:** At L5, the system is proposing and implementing enough work that single-agent sessions become a bottleneck. Claude-squad lets you run and monitor multiple agents simultaneously from one terminal.
...
**Verdict:** KEEP — at L5 you genuinely need multi-agent management. The AGPL license is the main concern for commercial contexts.
...
**What it actually does:** Visual interface for annotating and reviewing coding agent plans and code diffs. Generates shareable views of agent plans. Allows one-click feedback to agents. Designed for the L5 workflow where humans review proposals rather than writing code.

**Why it's in the workflow:** At L5, the human role shifts from writing code to reviewing agent proposals. Plannotator makes that review process visual and shareable — critical when agents are generating enough proposals that raw diff review becomes a bottleneck.
...
**Why it's in the workflow:** At L5, the system starts proposing new skills to install. Without a scanner, you risk installing skills that contain prompt injection or exfiltrate data. SkillSpector is the only security gate for the skill supply chain.
...
**ACMM Level:** L6
**Recommended at:** L6 — Fully Autonomous
...
**Why it's in the workflow:** At L6, agents merge PRs autonomously. Without an audit trail, there's no way to investigate what happened when something goes wrong. Bernstein provides the tamper-proof log and lineage tracking that makes autonomous operation auditable.
...
**Verdict:** CONDITIONAL — keep at L6 only. If you're not doing autonomous merges, this is premature. The small community is a risk, but the compliance angle is unique.
...
**Why it's in the workflow:** At L6, multiple agents operate concurrently. Without a work ledger, two agents can independently pick up the same issue and produce conflicting PRs. Beads is the semaphore that prevents this.
...
**Verdict:** KEEP — unique coordination function at L6. The high star count (24.5K) and active development validate the approach. The open issue count is high but proportional to adoption.

---

## Summary

| Tool | Level | Stars | Verdict | Key reason |
|------|-------|-------|---------|------------|
| context7 | L2 | 57K | **KEEP** | Only source of live, version-specific docs |
| reporails/cli | L2 | 59 | **CONDITIONAL** | Only if CLAUDE.md is complex; tiny community |
| code-review | L3 | Anthropic | **KEEP** | Confidence-scored review filters noise |
| pr-review-toolkit | L3 | Anthropic | **KEEP** | Dimension-specific agents catch what general review misses |
| claude-reflect | L3 | 1K | **KEEP** | Turns corrections into persistent rules |
| langfuse | L3 | 29K | **CONDITIONAL** | Only if running AI in production pipelines |
| headroom | L4 | 29K | **KEEP** | Directly solves context exhaustion at L4 |
| worktrunk | L4 | 5.5K | **CONDITIONAL** | Only if running parallel agent sessions |
| GSD | L4 | (superpowers) | **KEEP** | Structured verification prevents drift |
| feature-dev | L4 | Anthropic | **KEEP** | Single-feature structure, complements GSD |
| claude-squad | L5 | 7.8K | **KEEP** | Multi-agent management at L5 scale |
| plannotator | L5 | 6.2K | **CONDITIONAL** | Only if proposal review volume warrants visual UI |
| SkillSpector | L5 | 6.4K | **KEEP** | Only security gate for skill supply chain |
| bernstein | L6 | 576 | **CONDITIONAL** | Only at L6 with autonomous merges |
| beads | L6 | 24.5K | **KEEP** | Prevents duplicate work across agent fleet |
