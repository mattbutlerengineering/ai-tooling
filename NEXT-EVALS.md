...
Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 97 distinct values across these 452 leads (177 have zero overlap pressure; largest tie: 37) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P2 challenger** | overlaps a tool already in STACK | 124 | SKIP "redundant with `<incumbent>`", or leave at discovery-log |
...
| qwen-code | Implement | 27.6 | pressure 9, gap 7.6 | `/evaluate-tool qwen-code` |
...
| orca | Implement | 27.6 | pressure 9, gap 7.6 | `/evaluate-tool orca` |
...
## P2 challenger — 124 leads

_SKIP "redundant with `<incumbent>`", or leave at discovery-log._

_Listing 12 of 124 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
...
| HolyClaude | Implement | 17.6 | pressure 4, gap 7.6 | `/triage-lead HolyClaude` |
| Nimbalyst | Implement | 17.6 | pressure 4, gap 7.6 | `/triage-lead Nimbalyst` |
| agent-of-empires | Implement | 17.6 | pressure 4, gap 7.6 | `/triage-lead agent-of-empires` |
