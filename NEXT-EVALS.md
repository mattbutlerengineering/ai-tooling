...
The 446 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 108 distinct values across these 446 leads (187 have zero overlap pressure; largest tie: 27) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P2 challenger** | overlaps a tool already in STACK | 134 | SKIP "redundant with `<incumbent>`", or leave at discovery-log |
...
|------|-------|-------|-----|---------|
| opencode | Implement | 57.2 | pressure 25, gap 5.2 | `/evaluate-tool opencode` |
| cognee | Memory & Context | 44.2 | pressure 18, gap 6.2 | `/evaluate-tool cognee` |
| agent-browser | Verify | 41.9 | pressure 17, gap 5.9 | `/evaluate-tool agent-browser` |
| ECC | Implement | 41.2 | pressure 17, gap 5.2 | `/evaluate-tool ECC` |
| langfuse | Outer Loop | 40.8 | pressure 17, gap 6.8 | `/evaluate-tool langfuse` |
| claude-hud | Plan | 37.4 | pressure 15, gap 5.4 | `/evaluate-tool claude-hud` |
...
| mem0 | Memory & Context | 36.2 | pressure 14, gap 6.2 | `/evaluate-tool mem0` |
...
| tdd-guard | Review | 35.8 | pressure 14, gap 5.8 | `/evaluate-tool tdd-guard` |
| orca | Implement | 35.2 | pressure 14, gap 5.2 | `/evaluate-tool orca` |
| vet | Review | 33.8 | pressure 13, gap 5.8 | `/evaluate-tool vet` |
| aider | Implement | 33.2 | pressure 14, gap 5.2 | `/evaluate-tool aider` |
| agentmemory | Memory & Context | 30.2 | pressure 11, gap 6.2 | `/evaluate-tool agentmemory` |
| ghostsecurity/skills | Review | 29.8 | pressure 11, gap 5.8 | `/evaluate-tool ghostsecurity/skills` |
...
## P2 challenger — 134 leads

_SKIP "redundant with `<incumbent>`", or leave at discovery-log._

_Listing 12 of 134 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._

| Tool | Stage | Score | Why | Command |
|------|-------|-------|-----|---------|
| browser-use | Verify | 27.9 | challenges playwright · pressure 10, gap 5.9 | `/triage-lead browser-use` |
| ralph-claude-code | Implement | 27.2 | challenges GSD · pressure 10, gap 5.2 | `/triage-lead ralph-claude-code` |
| memU | Memory & Context | 26.2 | challenges claude-mem · pressure 9, gap 6.2 | `/triage-lead memU` |
| claude-octopus | Review | 25.8 | challenges code-review · pressure 9, gap 5.8 | `/triage-lead claude-octopus` |
...
| gastown | Implement | 23.2 | challenges claude-squad · pressure 8, gap 5.2 | `/triage-lead gastown` |
...
| diagnosing-bugs | Verify | 7.9 | ships inside `mattpocock/skills` · pressure 0, gap 5.9 | `/triage-lead diagnosing-bugs` |
