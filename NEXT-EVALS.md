...
The 566 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 107 distinct values across these 566 leads (243 have zero overlap pressure; largest tie: 42) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P2 challenger** | overlaps a tool already in STACK | 165 | SKIP "redundant with `<incumbent>`", or leave at discovery-log |
| **P3 backlog** | everything else | 371 | leave; stamp `**Last triaged:**` only |
...
| impeccable | Skills & Plugins | 32.4 | pressure 12, gap 6.4 | `/evaluate-tool impeccable` |
| ui-ux-pro-max | Skills & Plugins | 32.4 | pressure 12, gap 6.4 | `/evaluate-tool ui-ux-pro-max` |
...
## P2 challenger — 165 leads

_SKIP "redundant with `<incumbent>`", or leave at discovery-log._

_Listing 12 of 165 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
...
| openskills | Skills & Plugins | 24.4 | challenges skill-creator · pressure 8, gap 6.4 | `/triage-lead openskills` |
...
## P3 backlog — 371 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 371 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
