...
The 452 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 109 distinct values across these 452 leads (191 have zero overlap pressure; largest tie: 27) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P3 backlog** | everything else | 279 | leave; stamp `**Last triaged:**` only |
...
| ECC | Implement | 43.2 | pressure 18, gap 5.2 | `/evaluate-tool ECC` |
...
| spec-kit | Plan | 39.4 | pressure 16, gap 5.4 | `/evaluate-tool spec-kit` |
...
| tdd-guard | Review | 35.9 | pressure 14, gap 5.9 | `/evaluate-tool tdd-guard` |
| orca | Implement | 35.2 | pressure 14, gap 5.2 | `/evaluate-tool orca` |
| vet | Review | 33.9 | pressure 13, gap 5.9 | `/evaluate-tool vet` |
...
| ghostsecurity/skills | Review | 31.9 | pressure 12, gap 5.9 | `/evaluate-tool ghostsecurity/skills` |
...
| claude-octopus | Review | 25.9 | challenges code-review · pressure 9, gap 5.9 | `/triage-lead claude-octopus` |
...
## P3 backlog — 279 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 279 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
