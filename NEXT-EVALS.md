...
The 399 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 96 distinct values across these 399 leads (159 have zero overlap pressure; largest tie: 33) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P3 backlog** | everything else | 267 | leave; stamp `**Last triaged:**` only |
...
|------|-------|-------|--------------------|---------|
| opencode | Implement | 56.7 | pressure 24, gap 6.7 | `/evaluate-tool opencode` |
...
| ECC | Implement | 36.7 | pressure 14, gap 6.7 | `/evaluate-tool ECC` |
| OpenHands | Implement | 36.7 | pressure 14, gap 6.7 | `/evaluate-tool OpenHands` |
| goose | Implement | 36.7 | pressure 14, gap 6.7 | `/evaluate-tool goose` |
...
| pydantic-ai | Implement | 32.7 | pressure 12, gap 6.7 | `/evaluate-tool pydantic-ai` |
...
| sandcastle | Implement | 28.7 | pressure 10, gap 6.7 | `/evaluate-tool sandcastle` |
...
| agent-kit | Implement | 26.7 | pressure 9, gap 6.7 | `/evaluate-tool agent-kit` |
| aider | Implement | 26.7 | pressure 10, gap 6.7 | `/evaluate-tool aider` |
| gstack | Implement | 26.7 | pressure 9, gap 6.7 | `/evaluate-tool gstack` |
...
| orca | Implement | 26.7 | pressure 9, gap 6.7 | `/triage-lead orca` |
| ruflo | Implement | 26.7 | pressure 9, gap 6.7 | `/triage-lead ruflo` |
...
| gastown | Implement | 24.7 | pressure 8, gap 6.7 | `/triage-lead gastown` |
...
## P3 backlog — 267 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 267 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._

| Tool | Stage | Score | Why (pressure/gap) | Command |
|------|-------|-------|--------------------|---------|
| CopilotKit | Implement | 24.7 | pressure 8, gap 6.7 | `/triage-lead CopilotKit` |
| daytona | Implement | 24.7 | pressure 8, gap 6.7 | `/triage-lead daytona` |
| voltagent | Implement | 24.7 | pressure 8, gap 6.7 | `/triage-lead voltagent` |
| CLIProxyAPI | Implement | 22.7 | pressure 7, gap 6.7 | `/triage-lead CLIProxyAPI` |
| fast-agent | Implement | 22.7 | pressure 7, gap 6.7 | `/triage-lead fast-agent` |
| bifrost | Implement | 20.7 | pressure 6, gap 6.7 | `/triage-lead bifrost` |
| haystack | Implement | 20.7 | pressure 6, gap 6.7 | `/triage-lead haystack` |
| agent-sandbox | Implement | 18.7 | pressure 5, gap 6.7 | `/triage-lead agent-sandbox` |
| claude-code-router | Implement | 16.7 | pressure 4, gap 6.7 | `/triage-lead claude-code-router` |
| vercel-ai | Implement | 16.7 | pressure 4, gap 6.7 | `/triage-lead vercel-ai` |
| tambo | Implement | 14.7 | pressure 4, gap 6.7 | `/triage-lead tambo` |
| UI-TARS-desktop | Implement | 14.7 | pressure 3, gap 6.7 | `/triage-lead UI-TARS-desktop` |
