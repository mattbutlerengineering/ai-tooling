...
The 453 `discovery-log` leads, **derived** (not hand-maintained) from data already in the repo plus `repo-metadata.json`. Regenerate with `python3 triage.py`; do not edit between the markers.

Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 101 distinct values across these 453 leads (194 have zero overlap pressure; largest tie: 29) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| **P3 backlog** | everything else | 284 | leave; stamp `**Last triaged:**` only |
...
| langfuse | Outer Loop | 40.7 | pressure 17, gap 6.7 | `/evaluate-tool langfuse` |
...
| claude-octopus | Review | 25.9 | pressure 9, gap 5.9 | `/evaluate-tool claude-octopus` |
| gstack | Implement | 25.1 | pressure 9, gap 5.1 | `/evaluate-tool gstack` |
| qwen-code | Implement | 25.1 | pressure 9, gap 5.1 | `/evaluate-tool qwen-code` |
| ruflo | Implement | 25.1 | pressure 9, gap 5.1 | `/evaluate-tool ruflo` |
...
| Skill_Seekers | Skills & Plugins | 18.5 | challenges skill-creator · pressure 5, gap 6.5 | `/triage-lead Skill_Seekers` |
| SocratiCode | Plan | 17.2 | challenges codegraph · pressure 5, gap 5.2 | `/triage-lead SocratiCode` |

## P3 backlog — 284 leads

_leave; stamp `**Last triaged:**` only._

_Listing 12 of 284 — rerun `python3 triage.py` and read the source for the tail (no silent cap)._
...
| gptme | Implement | 21.1 | pressure 7, gap 5.1 | `/triage-lead gptme` |
