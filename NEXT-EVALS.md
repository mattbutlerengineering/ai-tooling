...
Leads are grouped into **bands**, not a single ranked list. Within a band the order is `2*overlap_pressure + stage_gap_weight + evidence_bonus` (see `next-evals.py`), but that score has only 104 distinct values across these 625 leads (266 have zero overlap pressure; largest tie: 49) — enough to pick a head, not to rank a tail. Leads already stamped `**Last triaged:**` sink within their band so each pass surfaces un-examined ones.
...
| vet | Review | 55.3 | pressure 23, gap 7.3 | `/evaluate-tool vet` |
...
| agnix | Review | 21.3 | challenges SkillSpector · pressure 6, gap 7.3 | `/triage-lead agnix` |
