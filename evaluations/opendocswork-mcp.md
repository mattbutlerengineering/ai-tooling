...
- **Identity is confused — naming and license both inconsistent.** The repo is `opendocswork-mcp`, the README titles it `office-oxide-mcp`, the install command is `cargo install office-oxide-mcp`, and several links point at a separate `Aimino-Tech/office-oxide-mcp` repo. The license is reported by GitHub as **GPL-3.0**, while the README badge claims **MIT/Apache-2.0**. GPL-3.0 is copyleft and materially different from MIT/Apache — this conflict must be resolved before any team relies on it, and it is a real adoption blocker.
