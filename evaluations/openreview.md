...
- **~~⚠️ No license.~~ Corrected 2026-08-05.** The original bullet read "no LICENSE file despite 'open-source' framing … default copyright means no granted right to use/modify/redistribute." The README declares **MIT** under a `## License` heading; what is absent is the root `LICENSE` file GitHub's detector reads. For something you are meant to fork and self-host, a README grant without the license text is thinner than a LICENSE file — worth resolving upstream, but not the blocker this bullet claimed.
...
**SKIP — beta, dormant, and platform-coupled.** Re-grounded on 2026-08-05: ~~SKIP (license)~~ — **the license ground is withdrawn.**

It read *"disqualified by this catalog's permissive-OSS adoption bar (#36, ADR 0001): the repo has no LICENSE file (re-verified 2026-06-22) — default copyright grants no reuse rights. License alone removes it from the adoptable set."* The repo declares **MIT** in a `## License` section of its README. GitHub's licensee detector reads a root `LICENSE` file and nothing else, so the re-verification confirmed the API's answer rather than the repo's terms (#372).

Note what that costs: this row's disposition was *stated* to rest on the licensing bar alone, so with that ground gone the SKIP has to be re-argued rather than merely re-worded. It survives, on the gates the same paragraph already recorded — **beta with no releases**, **dormant since 2026-03-06** (five months, on a PR-review bot whose value depends on current model behaviour), and **heavy Vercel-platform coupling** — against a Review stage where `PR-Agent` and the `code-review` plugin are mature, platform-neutral incumbents. That is a weaker SKIP than the one it replaces, and an honest one.

_Prior technical assessment retained for reference — it would otherwise be CONDITIONAL:_ adopt if you're on Vercel and want a self-hosted, on-demand PR-review bot that actually runs your tooling in a sandbox and can push trivial fixes, with extensible team skills. The sandboxed-execution + inline-suggestion + auto-commit design is strong and the Claude/AI-SDK foundation is current. But the significant gate is **beta status with no releases and (as of 2026-08-05) five months of quiet**, plus heavy Vercel-platform coupling. Pilot it; don't make it load-bearing yet. If you want a more turnkey, platform-neutral reviewer, PR-Agent or the Anthropic `code-review` plugin are alternatives.

Compared to neighbors: **PR-Agent** is a mature, platform-agnostic PR bot; **code-review**/**pr-review-toolkit** are Claude Code review plugins for local/agent use; **claude-code-action** wires Claude into GitHub Actions. OpenReview is the **Vercel-native, sandbox-executing, auto-fixing** PR bot — distinctive for running tooling and pushing fixes, distinctive also for its maturity gaps.

## Catalog entry

| Name | Type | One-liner | Problem it solves | Overlaps with |
|------|------|-----------|-------------------|---------------|
| [openreview](https://github.com/vercel-labs/openreview) | tool | Self-hosted AI PR-review bot (Vercel Labs) — mention @openreview, it reviews in a Vercel Sandbox with full repo access (runs linters/tests), posts inline suggestions, and can auto-commit fixes; powered by Claude (⚠️ beta, dormant since 2026-03) | Want on-demand PR review that runs your actual tooling and can push trivial fixes, not just diff-read | PR-Agent, code-review, claude-code-action |
