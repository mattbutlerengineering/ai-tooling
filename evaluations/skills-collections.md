...
**Why:** Best philosophy (composable, small, hackable), solves the most impactful problem (agent-human alignment via grilling), and introduces domain modeling via `CONTEXT.md` which no other collection offers. 130k stars validates the approach. The deliberate restraint — fewer skills, each one battle-tested — aligns with the ACMM insight that fewer tools with better feedback loops beats more tools with no infrastructure.
