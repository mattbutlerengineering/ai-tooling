...
- `python3 next-evals.py` regenerates `NEXT-EVALS.md` — a ranked promotion queue answering "what do we evaluate next", derived (not hand-maintained) from each `discovery-log` COMPARISON row: `score = 2*overlap_pressure + stage_gap_weight + evidence_bonus` (overlap_pressure = other catalog rows citing the tool in "Overlaps with", reusing `audit-evals.py`'s `overlap_pressure_map`; stage_gap_weight = `10*(1 - Validated/Tools)` for the row's stage, hungriest highest; evidence_bonus = +2 for `REVIEW`). The queue *selects*; humans/attended agents run `/evaluate-tool`. Weights are a starting heuristic (named constants at the top of the script); `--check` gates it in CI. Only `discovery-log` rows are candidates — evaluated verdicts are never queued.
