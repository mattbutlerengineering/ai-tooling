...
- `CATALOG.md` — flat inventory of 634 tools, organized by 13 categories with overlap markers
...
- `python3 triage.py` regenerates `NEXT-EVALS.md` — the leads grouped into **priority bands**, each declaring what an agent may conclude about it. Derived from the verdict data plus `repo-metadata.json`; `--check` gates it in CI. Bands: **P0 measure** (score head — the only band that may reach ADOPT, human or `eval-runner` only), **P1 successor-check** (archived: repoint to a successor or SKIP "archived, no successor" — archived usually means *moved*, so it is never an auto-SKIP), **P2 challenger** (overlaps a STACK pick: SKIP "redundant with X" or leave), **P3 backlog**, **P4 mechanical-skip** (a *vendored* Type — `skill`/`plugin`, whose text is copied into the consuming repo — under `NONE`/`AGPL`/`GPL`/`CC-BY-SA`/`EUPL`). The vendored scoping is the point: copyleft on a CLI you merely *run* imposes nothing, and a naive copyleft rule would SKIP `anthropics/claude-code` and `firecrawl` while letting a CC-BY-SA **skill** through. `NOASSERTION` never disposes a lead — it means GitHub could not parse the LICENSE file, not that one is absent.
  ...
  - A `discovery-log` eval still carries a `## Verdict` line — per COMPARISON.md's legend it is the eval's *tentative read, notes rather than a recommendation*. Promoting such a lead to a real verdict is the whole job. Key on the **headline** verdict (`ev.verdict`), never `verdict_set`: `trailofbits/skills` reads "Held at CONDITIONAL rather than ADOPT", so its set contains `ADOPT` while its verdict does not.
