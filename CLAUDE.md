...
- `CATALOG.md` — flat inventory of 505 tools, organized by 13 categories with overlap markers
...
- `python3 audit-evals.py` runs the gating detectors (A install, B fabrication, D verdict sync, G comparison consistency, J stack-derivation drift) and exits non-zero on any problem (gate it in CI/pre-commit); opt-in detectors C/E/F/I and `--selftest` add more checks:
